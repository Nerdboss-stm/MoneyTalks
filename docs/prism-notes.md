...
- OBSERVED 2026-09-05 (ingest key, header `X-PRISMtrace-Key`, none of these routes are on the docs page): `GET /api/traces/{id}` → 200 with the stored trace: `id, trace_id, project_id, session_id, model, input_messages, output_message, latency_ms, token_count_input, token_count_output, cost_usd, metadata` (echoed as sent), `guardrail_flags, pii_redacted, user_identifier, related_traces[], spans[], db_spans[], created_at, evaluation` (§5). `GET /api/traces?project_id=&session_id=&limit=` → `{page, page_size, traces[]}`, each entry with `evaluation` and `evaluation_status` (`"complete"` observed). `GET /api/sessions/{session_id}?project_id=` → `{session_id, traces[], agent_run_id, agent_run_status, agent_run_agent_name, internal_trace_count}`. `GET /api/traces/{id}/analysis` and `/score` → 404. UNVERIFIED: a read for runs or spans on their own (`GET /api/runs` not tried beyond the session object above), and whether a read-scoped key differs from the ingest key on these routes.
- UNVERIFIED: dashboard page URL shapes. No path for a project, session, trace or run page is documented; the only verified page is the host root `https://prism.blockconvey.com`, so every product link goes there and is labelled "Open PRISM" (`prism_util.links`).
- UNVERIFIED: metadata on a trajectory. `[SDK src]` the `submit_trajectory` payload has no `metadata` field (`project_id, conversation_id, request_id, agent_id, agent_name, steps[], total_duration_ms, final_status, model`), so `failure_class` travels in the final step's `label` (`answer · failure_class=…`) for trajectories, in `/api/traces` `metadata` for traces, and in the spans-ingest body `metadata` (documented `metadata?`) for span batches.
...
- Read path for scores: dashboard ("Click any trace for the full conversation, the automatic analysis ..."). Project-level: `GET /api/setup-doctor?project_id=…` returns `live_connected`, per-stage `steps[]` (`title`, `status` in `done|pending`, `detail`) and `blocked_step` `[SDK src verify.py]`.
- OBSERVED 2026-09-05 per-trace read (not on the docs page, see §4): `GET /api/traces/{id}` carries `evaluation = {id, customer_satisfaction, response_quality, intent_detected, flag_for_review, flag_reason, industry, industry_scores, created_at}` once analysis has run. Observed values: the `explain-v2-01` procurement trace `c8248169…` → satisfaction 75, quality 82, `flag_for_review: false`, intent `budget_variance_inquiry`; the `explain-v1-01` procurement trace `eb72827d…`, whose verify.py result in `metadata.verifier` is `ok: false` with nine unverifiable figures → satisfaction 82, quality 85, `flag_for_review: false`. PRISM did not flag the hallucinated figures; verify.py did, before speech. The product reads `response_quality` as "SCORED n" and `flag_for_review` as FLAGGED / NO FLAG (`prism_util.read_verdict`, polled for 15 s after each answer; past that the room shows RECORDED only). UNVERIFIED: the shape of `evaluation` before analysis lands (the reader treats a missing or null object as "not yet scored").
