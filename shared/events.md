...
- `prism.verdict` `{trace_id, status: "scored"|"recorded"|"unrecorded", score?, satisfaction?, flagged?, reason?}` — exactly once per answer, in the background after `agent.answer`. `scored` when `GET /api/traces/{id}` returned an `evaluation` within 15 s (`score` = `response_quality`, `flagged` = `flag_for_review`); `recorded` when the trace exists but analysis had not landed; `unrecorded` when no trace was sent. The room appends to the RECORDED line "PRISM · SCORED 84 · NO FLAG", "PRISM · FLAGGED" (the word FLAGGED in #E6E6E3) or "PRISM · RECORDED · <trace_id[:8]>", Plex Mono 10px. Copy never implies PRISM blocked a sentence: verify.py gates before speech; PRISM is the independent record and auditor.
- Trace and trajectory metadata carries `failure_class`: `hallucinated_figure` when the meeting verifier failed the spoken answer, `stale_status_contradiction` for a MANDATE v1 executor report that claims compliance over an execution inside a bound mandate window, else `none`.
